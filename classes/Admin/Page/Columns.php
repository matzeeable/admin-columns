...
class Columns extends Page implements Enqueueables, Helpable, Admin\ScreenOptions {
	...
	public function show_read_only_notice( ListScreen $list_screen ) {
		if ( $list_screen->is_read_only() ) {
			$message = sprintf( __( 'The columns for %s are read only and can therefore not be edited.', 'codepress-admin-columns' ), '<strong>' . esc_html( $list_screen->get_title() ? $list_screen->get_title() : $list_screen->get_label()->get_plural() ) . '</strong>' );
